...
class _MenteeRegistrationPageState extends State<MenteeRegistrationPage> {
  ...
  final List<String> _universidadesBanco = [
    "FATEC Ipiranga", "FATEC São Paulo", "USP", "UNESP", "UNICAMP", "Outra"
  ];
  ...
  void _validarEEnviar() {
    ...
    if (!_canCommit || !_authorized || _disponibilidade == null || _momentoCarreira == null) {
      _showSnackBar("Preencha as seleções obrigatórias e aceite os termos.");
      return;
    }

    Navigator.push(context, MaterialPageRoute(builder: (context) => const TermSigningPage()));
  }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildRow([
                          ...
                          _buildField("WhatsApp", "(00) 00000-0000", controller: _phoneController, required: true, isNumber: true),
                        ]),
                        _buildRow([
                          _buildField("CPF", "000.000.000-00", controller: _cpfController, required: true, isNumber: true),
                          _buildField("Senha", "**********", controller: _passwordController, required: true, obscure: true),
                        ]),
                        _buildRow([
                          _buildReadOnlyField("Data de Nascimento", "DD/MM/AAAA", _birthDateController, onTap: () => _selectDate(context)),
                          _buildField("LinkedIn (Opcional)", "linkedin.com/in/perfil", controller: _linkedinController),
                        ]),
                        _buildRow([
                          _buildDropdownField("Gênero", ["Feminino", "Masculino", "Não-binário", "Outro"], _gender, (v) => setState(() => _gender = v)),
                          _buildDropdownField("Raça/etnia", ["Amarela", "Branca", "Indígena", "Parda", "Preta"], _race, (v) => setState(() => _race = v)),
                        ]),
                        ...
                        _buildDropdownField("Área do Curso (STEM)", ["TI e Dados", "Engenharia", "Ciências", "Matemáticas", "Arquitetura", "Outros"], _formacaoAcademica, (v) => setState(() => _formacaoAcademica = v), isRequired: true),
                        _buildRow([
                          _buildField("Nome do Curso", "Ex: ADS", controller: _courseController, required: true),
                          _buildField("Ano ou Semestre Atual", "Ex: 4º Semestre", controller: _periodoController, required: true),
                        ]),

                        const SizedBox(height: 35),
                        _buildSectionTitle("Matching Inteligente"),
                        _buildChoiceBox("Qual seu momento atual?", ["Buscando transição de carreira", "Buscando 1º estágio/emprego", "Já trabalho na área", "Apenas estudando"], _momentoCarreira, (v) => setState(() => _momentoCarreira = v)),
                        const SizedBox(height: 25),
                        _buildChoiceBox("Onde quer focar na mentoria?", ["Hard Skills (Técnico)", "Soft Skills (Comportamental)", "Networking e Carreira"], _focoPrincipal, (v) => setState(() => _focoPrincipal = v)),
                        
                        const SizedBox(height: 30),
                        const Text("Idiomas (Mínimo 2)", style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14, color: Colors.black87)),
                        const SizedBox(height: 12),
                        _buildMultiSelect(["Português", "Espanhol", "Inglês"], _idiomasSelecionados),
                        
                        const SizedBox(height: 30),
                        const Text("Competências para desenvolver (Mín 2, Máx 4)", style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14, color: Colors.black87)),
                        const SizedBox(height: 12),
                        _buildMultiSelect(_opcoesCompetencias, _competenciasDesejadas, maxSelection: 4),
                        
                        const SizedBox(height: 30),
                        const Text("Seus Hobbies (Mínimo 2)", style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14, color: Colors.black87)),
                        const SizedBox(height: 12),
                        _buildMultiSelect(_opcoesHobbies, _hobbiesSelecionados),

                        const SizedBox(height: 35),
                        _buildChoiceBox("Disponibilidade para sessões", ["Manhã", "Tarde", "Noite"], _disponibilidade, (v) => setState(() => _disponibilidade = v)),
                        ...
                        _buildCheckboxTile(
                          ...
                          subtitle: "Declaro que posso me comprometer com sessões quinzenais.",
                          onChanged: (v) => setState(() => _canCommit = v!),
                        ),
                        _buildCheckboxTile(
                          value: _authorized,
                          title: "Autorização de Dados",
                          subtitle: "Autorizo o compartilhamento do perfil para fins de matching.",
                        ...
                        Center(
                          child: SizedBox(
                            ...
                            child: FilledButton(
                              ...
                              child: const Text("Finalizar Inscrição", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                            ),
                          ),
                        ),
                      ],
  ...
  Widget _buildHeader() {
    return Center(
      child: Column(
        children: [
          Text("Cadastro de Mentorada", style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: brandColor)),
          const SizedBox(height: 8),
          const Text("Preencha os campos para o matching inteligente", style: TextStyle(color: Colors.black54, fontSize: 16)),
        ],
      ),
    );
  }
  ...
  Widget _buildField(String label, String hint, {TextEditingController? controller, bool required = false, bool isEmail = false, bool isNumber = false, bool obscure = false}) {
    return Padding(
        ...
        children: [
          ...
          TextFormField(
            ...
            decoration: _inputDecoration(hint).copyWith(
              suffixIcon: obscure 
                ? IconButton(icon: Icon(_senhaVisivel ? Icons.visibility : Icons.visibility_off, color: brandColor, size: 20), onPressed: () => setState(() => _senhaVisivel = !_senhaVisivel))
                : null
            ),
            validator: (value) {
  ...
  Widget _buildAutocompleteUniversity() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text("Universidade / Instituição", style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: Colors.black87)),
        const SizedBox(height: 8),
        Autocomplete<String>(
          optionsBuilder: (textValue) => _universidadesBanco.where((opt) => opt.toLowerCase().contains(textValue.text.toLowerCase())),
          ...
          fieldViewBuilder: (ctx, ctrl, node, onComplete) => TextFormField(
            ...
            validator: (v) => (_universidadeSelecionada == null || v!.isEmpty) ? "Selecione da lista" : null,
          ),
        ),
      ],
    );
      ...
      children: options.map((o) {
        ...
        return FilterChip(
          ...
          onSelected: (v) {
            setState(() {
              if (v) { 
                if (maxSelection == null || target.length < maxSelection) target.add(o); 
              } else { 
                target.remove(o); 
              }
            });
          ...
          labelStyle: TextStyle(color: sel ? petroleo : textDark, fontWeight: sel ? FontWeight.bold : FontWeight.normal),
          shape: RoundedRectangleBorder(side: BorderSide(color: sel ? petroleo : greyBorder), borderRadius: BorderRadius.circular(12)),
        );
      }).toList(),
    );
  }

  Widget _buildCheckboxTile({required bool value, required String title, required String subtitle, required Function(bool?) onChanged}) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          TextFormField(
            ...
            decoration: _inputDecoration(hint).copyWith(suffixIcon: Icon(Icons.calendar_today, color: brandColor, size: 18)),
  ...
  Widget _buildDropdownField(String label, List<String> items, String? current, Function(String?) onChanged, {bool isRequired = false}) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 20),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          DropdownButtonFormField<String>(
            ...
            onChanged: onChanged,
            validator: (val) => (isRequired && val == null) ? "Obrigatório" : null,
          ),
        ],
      ),
    );
