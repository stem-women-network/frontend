...
class _MentorRegistrationPageState extends State<MentorRegistrationPage> {
  ...
  final TextEditingController _outroFormacaoController =
  ...
  void _irParaTermos() {
    ...
    if (_competenciasExpertise.length < 2 ||
        _hobbiesSelecionados.length < 2 ||
        _idiomasSelecionados.length < 2 ||
        _focosEnsinoSelecionados.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "Selecione os focos de mentoria e ao menos 2 competências e hobbies.",
          ),
        ),
      );
      return;
    ...
    if (_compromisso14Sessoes != "Sim") {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "Você precisa confirmar o compromisso com as 14 sessões.",
          ),
        ),
      );
      return;
    ...
    if(_disponibilidade == null){
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "Você precisa escolher a sua disponibilidade",
          ),
        ),
      );
      return;
  ...
  void _finalizarCadastroReal() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          "Suas informações foram unificadas e registradas com sucesso. Entraremos em contato em breve.",
        ),
        actions: [
          FilledButton(
  ...
  Widget _buildForm1() {
    return Column(
      ...
      children: [
        ...
        if (_formacaoAcademica == "Outros")
          Padding(
            padding: const EdgeInsets.only(top: 15),
            child: _buildField(
              "Especifique sua formação",
              "Qual área?",
              controller: _outroFormacaoController,
            ),
          ),
        const SizedBox(height: 20),
        _buildChoiceBox(
          "Comprometimento quinzenal?",
          ["Sim", "Não", "Talvez"],
          _compromissoQuinzenal,
          (v) => setState(() => _compromissoQuinzenal = v),
        ),
      ],
    );
  }
  ...
  Widget _buildForm2() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 20),
        _buildDropdown("Área de Atuação *", [
          "TI e Dados",
          "Engenharia",
          "Ciências",
          "Arquitetura",
          "Outros",
        ], (v) => setState(() => _areaAtuacao = v)),
        const SizedBox(height: 20),
        _buildChoiceBox(
          "Já foi mentor antes?",
          ["Sim", "Não"],
          _foiMentor,
          (v) => setState(() => _foiMentor = v),
        ),
        const SizedBox(height: 10),
        _buildChoiceBox(
          "Já foi mentorado antes?",
          ["Sim", "Não"],
          _foiMentorado,
          (v) => setState(() => _foiMentorado = v),
        ),
        const SizedBox(height: 35),
        _buildSectionTitle("Matching Inteligente"),
        _buildChoiceBox(
          "Qual perfil deseja ajudar? *",
          [
            "Transição de carreira",
            "1º estágio/emprego",
            "Já na área",
          ...
          _perfilInteresse,
          (v) => setState(() => _perfilInteresse = v),
        ),
        const SizedBox(height: 25),
        const Text(
          "Foco da mentoria (Múltiplo) *",
        ...
        const SizedBox(height: 25),
        const Text(
          "Expertise (Mín 2, Máx 4) *",
          style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14),
        ),
        _buildMultiSelect(
          _opcoesExpertise,
          _competenciasExpertise,
          maxSelection: 4,
        ),
        const SizedBox(height: 25),
        const Text(
        ...
        const SizedBox(height: 20),
        _buildChoiceBox(
          "Pode se comprometer com 14 sessões? *",
          ["Sim", "Não"],
          _compromisso14Sessoes,
          (v) => setState(() => _compromisso14Sessoes = v),
        ),
      ],
    );
  }
  ...
  Widget _buildAutocompleteCity() => Column(
    ...
    children: [
      ...
      const SizedBox(height: 8),
      Autocomplete<String>(
        optionsBuilder: (val) => _cidadesPrincipais.where(
          (s) => _removeAcentos(
            s.toLowerCase(),
          ).contains(_removeAcentos(val.text.toLowerCase())),
        ),
        onSelected: (s) => setState(() => _cidadeController.text = s),
        fieldViewBuilder: (ctx, ctrl, node, onComplete) => TextFormField(
          controller: ctx
              .findAncestorStateOfType<_MentorRegistrationPageState>()!
              ._cidadeController,
          focusNode: node,
  ...
  Widget _buildAutocompleteState() => Column(
    ...
    children: [
      ...
      const SizedBox(height: 8),
      Autocomplete<String>(
        optionsBuilder: (val) => _estadosBrasil.where(
          (s) => _removeAcentos(
            s.toLowerCase(),
          ).contains(_removeAcentos(val.text.toLowerCase())),
        ),
      ...
      children: _emailDomains
          .map(
            (d) => InkWell(
              onTap: () => setState(
                () =>
                    _emailController.text = _emailController.text.contains("@")
                    ? _emailController.text.substring(
                            0,
                            _emailController.text.indexOf("@"),
                          ) +
                          d
                    : _emailController.text + d,
              ),
              child: Container(
  ...
  Widget _buildSubmitButton() => SizedBox(
    ...
    child: FilledButton(
      onPressed: _currentStep == 1
          ? _validarETriagem
          : () {
              final response = authService.registerMentor(
                ...
                birthDate: _dataNascController.text,
                city: _cidadeController.text,
                gender: _genero!,
                formacao: _formacaoAcademica!,
                state: _estado!,
                race: _raca!,
                cargoAtual: _cargoController.text,
                areaAtuacao: _areaAtuacao!,
                wasMentor: _foiMentor == "Sim",
                wasMentee: _foiMentorado == "Sim",
                menteeProfile: _perfilInteresse!,
                mentoringGoal: _focosEnsinoSelecionados,
                languages: _idiomasSelecionados,
                skills: _competenciasExpertise,
                hobbies: _hobbiesSelecionados,
                availability: _disponibilidade!,
                help: _ajudaController.text,
      ...
      child: Text(
        _currentStep == 1 ? "VALIDAR E CONTINUAR" : "FINALIZAR INSCRIÇÃO",
