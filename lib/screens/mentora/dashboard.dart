...
class _MentoraDashboardState extends State<MentoraDashboard> {
  ...
  Future getCurrentMentee() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    var token = prefs.getString("token");
    token ??= "";
    return _mentorService.getCurrentMentee(token: token);
  }

  @override
  void initState() {
    _fetchData = getCurrentMentee();
    super.initState();
  }

  Future<void> _abrirMeet() async {
    final Uri url = Uri.parse(
      _linkMeet.startsWith('http') ? _linkMeet : 'https://$_linkMeet',
    );
                ...
                Row(
                  ...
                  children: [
                    _buildMenteeAction(
                      ...
                      () => Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => ChatPage(otherId: mentee["id"], actor:"mentor")),
                      ),
                    ),
                    _verticalDivider(),
                    _buildMenteeAction(
          ...
          children: [
            ...
            Text(
              ...
              style: TextStyle(
                ...
                color: isPrimary ? petroleo : Colors.black54,
              ),
            ),
          ],
        ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            "Valide a presença e o conteúdo.",
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            "$_dataProxima às $_horarioProxima",
      ...
      children: [
        ...
        _buildWideActionBtn(
          ...
          onTap: () => Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => const MentorFirstContactPage(),
            ),
          ),
        ),
        _buildWideActionBtn(
          ...
          onTap: () => Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const MenteeHistoryPage()),
          ),
        ),
        _buildWideActionBtn(
          Icons.folder_shared,
          ...
          onTap: () => Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const MentorTrainingPage()),
          ),
        ),
        _buildWideActionBtn(
          Icons.support_agent,
  ...
  Widget _buildStyledLabel(String t) => Padding(
  ...
  Widget _buildStyledField(
    ...
    onTap: () async {
      if (isDate) {
        ...
        if (p != null) c.text = "${p.day}/${p.month}/${p.year}";
      } else if (isTime) {
        TimeOfDay? p = await showTimePicker(
          context: context,
          initialTime: TimeOfDay.now(),
        );
        if (p != null) c.text = p.format(context);
      }
    },
    decoration: InputDecoration(
      hintText: h,
      prefixIcon: Icon(i, color: petroleo, size: 18),
      filled: true,
      fillColor: inputGrey,
  ...
  void _showSupportDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text("Contatar suporte?"),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
